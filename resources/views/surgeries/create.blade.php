...
<div class="container-fluid py-2">
    ...
    @if ($errors->any())
        <div class="alert alert-danger alert-dismissible fade show mb-2 py-2" role="alert">
            <div class="fw-bold mb-1"><i class="fas fa-exclamation-triangle me-1"></i> تعذر حفظ العملية بسبب الأخطاء التالية:</div>
                    ...
                    <div class="row g-2 mb-2 align-items-center">
                        <div class="col-md-9">
                            <div class="px-3 py-2 bg-light rounded border d-flex justify-content-between align-items-center small flex-wrap gap-2">
                                ...
                                <div>
                                    <span class="text-muted">أجرة الليلة الأولى (تثبيت الحجز):</span> 
                                    <b id="room_total_fee" class="badge bg-success fs-6 ms-1">0 د.ع</b>
                                </div>
                            </div>
                            <div class="mt-1 text-muted small" style="font-size: 0.76rem;">
                                <i class="fas fa-info-circle text-primary me-1"></i>
                                <strong>النظام الفندقي:</strong> الليلة الأولى تغطي المريض حتى <strong>12:00 ظهراً</strong> من اليوم التالي. أي ليالٍ إضافية تُحسب تلقائياً عند إجراء الخروج (عادية: مجانية أول ليلة | VIP: 100,000 د.ع | VVIP: 200,000 د.ع).
                            </div>
                        </div>
    ...
    window.scanFromDevice = async function() {
        ...
        try {
            ...
            reader.onloadend = function() {
                ...
                if (previewTxt) previewTxt.innerHTML = '<i class="fas fa-camera me-1"></i> تم سحب الصورة من السكانر بنجاح';
    ...
    if (referralInput) {
        referralInput.addEventListener('change', function(e) {
            if (this.files && this.files[0]) {
                const previewTxt = document.getElementById('preview_text');
                if (previewTxt) previewTxt.innerHTML = '<i class="fas fa-file-alt me-1"></i> تم اختيار الملف: ' + this.files[0].name;
                if (previewContainer) previewContainer.style.display = 'block';
            }
        });
    }
