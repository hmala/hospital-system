# حالة المشروع والمتابعة الحالية (2026-09-20)

## 🏥 نظام إدارة الصيدلية المتكاملة (Pharmacy System)
- **الخطوة 1 (مكتملة ✅)**: قاعدة البيانات، الجداول الستة (`medicines`, `medicine_alternatives`, `pharmacy_services`, `medicine_batches`, `pharmacy_sales`, `pharmacy_sale_items`)، وموديلات Eloquent مع خوارزمية FEFO وفتح العلب عند سحب الأشرطة.
- **الخطوة 2 (مكتملة ✅)**: دليل الأدوية والمستلزمات، إدارة الخدمات الصيدلانية، استيراد ملفات Excel/CSV، ربط البدائل العلمية، والصلاحيات والأدوار.
- **الخطوة 3 (مكتملة ✅)**: لوحة متابعة الشحنات والصلاحيات (FEFO Dashboard)، مراقبة المخزون المعرض للتلف، وتوريد وحجر الوجبات.
- **الخطوة 4 (مكتملة ✅)**: واجهة نقطة البيع السريعة والصرف (Dynamic POS)، قارئ الباركود، التبديل بين بيع العلبة والشريط، احتساب استقطاع الضمان (الفئات A إلى I)، تعليق واستئناف الفواتير (Hold/Resume)، وطباعة الوصل الحراري 80mm.

### 🔜 الخطوة التالية:
- **الخطوة 5**: الربط مع عيادات الاستشارية (سحب وصفات الأطباء المسجلة في الزيارات تلقائياً إلى نقطة بيع الصيدلية)، ودعم تحصيل المبالغ المحولة إلى الكاشير المركزي.

---

# أرشيف التحديثات السابقة:

### 🔧 ملخص التحديثات المنفذة اليوم (2026-09-02):

1. **نظام تعديل وحذف فواتير المشتريات ومزامنة المخزون (`purchases`)**:
   - مسار وشاشة تعديل كاملة: `purchases/{purchase}/edit` (`resources/views/purchases/edit.blade.php`).
   - دعم تعديل المورد، رقم الفاتورة، الكميات، الأسعار، وتواريخ الانتهاء للمواد، مع إمكانية إضافة وحذف بنود.
   - مزامنة المخزون التلقائية: تعديل كميات الوجبات المخزنية `StockBatch` وحركات `StockMovement` بالتوازي.
   - حماية حذف الفاتورة: التحقق من عدم وجود حركات صرف/نقل تمت على الوجبات قبل الحذف.
   - أزرار سريعة في جدول المشتريات (`purchases/index`): عرض 👁️، تعديل ✏️، حذف 🗑️.
   - أزرار في صفحة الفاتورة (`purchases/show`): تعديل ✏️، طباعة باركودات 🖨️، إرجاع للمخزن ↩️.

2. **إعادة تصميم واجهة جدول العمليات الجراحية (`surgeries/index`)**:
   - تحسين الجدول ليصبح واسعاً ومريحاً للعين (`padding: 12px 14px`).
   - استبدال القوائم المنسدلة المقصوصة بأزرار مباشرة: (تفاصيل، تعديل، طباعة، حذف).
   - تمييز الغرف بأوسمة فندقية ملونة (عادية، VIP، VVIP).
   - توسيع صلاحية الحذف والتعديل لتشمل موظفي الاستعلامات (`inquiry_staff`).

3. **نظام حساب أجور ومبيت الغرف الفندقي (`Room Stay Calculation`)**:
   - احتساب الليلة الأولى من وقت الدخول حتى 12:00 ظهراً لليوم التالي بتوقيت بغداد، ثم إضافة رسم الليلة عن كل يوم إضافي.
   - ربط الحساب المباشر بنموذج الدفع في الكاشير `cashier/surgeries/payment-form`.

4. **توليد حسابات الأطباء الخارجيين تلقائياً**:
   - توليد إيميل متسلسل `ext101@hospital.com` مع كلمة مرور موحدة `Doctor@123`.
   - تقليص قوائم Select2 الطويلة لشريط تمرير أنيق `max-height: 200px`.

5. **تحديث شجرة المعرفة AST (`graphify`)**:
   - تحديث شامل لـ 696 ملفاً بنسبة 100%.
...
### 16. نظام استرجاع مبالغ العمليات وتبسيط واجهات الحسابات والصندوق

- **إرجاع المبالغ الزائدة (Refund):** إضافة مسار وزر مخصص لمعالجة الفائض المالي عندما يتم تخفيض أو تعديل سعر العملية الجراحية بعد الدفع الفعلي.
- **تحديث واجهة مراجعة المحاسب:** إعادة تصميم `review-form.blade.php` ليكون العرض جانباً إلى جنب لمراجعة الحسابات والتفاصيل وتحديث الحسابات تلقائياً.
- **تبسيط الجداول:** تحسين واجهة قائمة العمليات لأمناء الصناديق والمحاسبين للتخلص من البيانات المتداخلة وعرض قائمة ملخصة.
...
**Views:**
- `resources/views/cashier/surgeries/index.blade.php` (محدث)
- `resources/views/cashier/surgeries/payment-form.blade.php` (محدث)
- `resources/views/accountant/surgeries/index.blade.php` (محدث)
- `resources/views/accountant/surgeries/review-form.blade.php` (محدث)

**Routes:**
- `routes/web.php` (محدث)

### 🔜 المهام المقترحة للجلسة القادمة:
1. اختبار عملية إرجاع المبالغ الزائدة (`refund`) للعمليات الجراحية عند الصندوق والتأكد من تعديل حالة الدفع وسجل الحسابات.
2. اختبار نموذج مراجعة المحاسب للتأكد من حساب الإجماليات بشكل مباشر وجنباً إلى جنب مع السجل التاريخي.
3. مراجعة واجهات الصندوق والمحاسب المبسطة للتأكد من سهولة ووضوح البيانات المعروضة.
